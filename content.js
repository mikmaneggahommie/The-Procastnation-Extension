...
if (window.__CURE_VAULT_INSTANCE__) {
    console.log('[Cure] Instance already exists, preventing duplicate.');
    throw new Error('CureVault already initialized');
}
...
class CureVault {
    constructor() {
        ...
        if (window.__CURE_VAULT_INSTANCE__) {
            console.warn('[Cure] Duplicate instance detected, aborting.');
            return window.__CURE_VAULT_INSTANCE__;
        }
        ...
        // Delayed to end of constructor to ensure all methods and properties are bound.
        ...
        document.addEventListener('visibilitychange', () => {
            if (!document.hidden) {
                ...
                if (this.isTypingChallengeActive) {
                    // FIX 116: Remove strict progress check.
                    // User wants to see the reset notification even if they haven't typed yet.
                    // Any abandonment of the challenge screen should trigger the warning.
    ...
    evaluateAllTriggers() {
        ...
        // FIX 107: Progress Reset Flow Restoration.
        // If a reset is pending, we MUST land on the Decision screen (Time's Up)
        // instead of the typing page or skipping the overlay entirely.
        const needsReset = sessionStorage.getItem(`cure_needs_reset_${window.location.hostname}`) === 'true';
        if (needsReset) {
            // Note: We don't remove the flag here; renderDecisionScreen does it to show the toast.
            this.stateHardLock('limit', true);
        ...
        return new Promise(resolve => {
            ...
            if (lockResponse && lockResponse.unlocked === true) {
                ...
                this.syncDailyStats().then((response) => {
            ...
            chrome.storage.local.get([key], (result) => {
                ...
                if (saved) {
                    try {
                        ...
                        if (sessionStorage.getItem(`cure_sticky_unlocked_${window.location.hostname}`) === 'true') {
                            this.stickyUnlocked = true;
                        }
                    } catch (e) {
    ...
    stateMonitor() {
        ...
        const runTick = () => {
            ...
            // FIX 93: Fail-safe Polling for Progress Reset
            // If visibilitychange failed to re-render, this will catch it.
            if (this.isTypingChallengeActive) {
                const needsReset = sessionStorage.getItem(`cure_needs_reset_${window.location.hostname}`) === 'true';
                if (needsReset) {
                    this.renderTypingLock( document.getElementById(this.overlayId) || this.ensureShadow() );
                }
            }
            ...
            if (hasOverlay && !hardLockEffective && !pauseEffective) {
                chrome.runtime.sendMessage({ action: 'getLockState', hostname: window.location.hostname }, (res) => {
                    if (!res || !res.locked) {
                        this.removeOverlay();
            ...
            if (remindersOn && deltaSecs >= 1) {
                ...
                if (this.activeSeconds % 30 === 0 && this.isContextValid()) {
                    chrome.runtime.sendMessage({ action: 'trackLaunch', hostname: window.location.hostname }, (launchRes) => {
                        if (!launchRes) return;
    ...
    unlockSession(mins) {
        ...
        if (this.isContextValid()) {
            ...
            chrome.runtime.sendMessage({ action: 'finishChallenge', hostname: window.location.hostname }, (response) => {
    ...
    confirmUnlock(mins) {
        ...
        if (this.settings.unlockRewardType === 'session' || this.settings.unlockRewardType === 'unlimited') {
             chrome.runtime.sendMessage({ action: 'startRewardSession', hostname: window.location.hostname }, () => {
                this.mode = 'up'; 
                this.activeSeconds = 0;
                location.reload(); 
            });
    ...
    showToast(msg, type = 'info') {
        ...
        // If we are calling showToast, we WANT it shown, regardless of frame status.
        // This fixes visibility in test environments and complex iframe-based apps.
        const root = this.ensureShadow();
        ...
        popout.className = `cure-popout ${type}`;
        popout.dataset.type = type;

        // Inner HTML with message and close button
        popout.innerHTML = `
            <span>${msg}</span>
            <button class="cure-popout-close">✕</button>
        `;

        // Fix 113: Always append as last child of shadow root to ensure it wins 
        // the stacking context even on sites with complex shadow injections.
        root.appendChild(popout);
        
        // Fix 111: Use the absolute maximum z-index to beat our own internal overlay (100)
        // AND any other potentially conflicting elements in the shadow root.
        popout.style.zIndex = '2147483647';

        // Animate in with a slightly more robust delay than rAF
        setTimeout(() => {
        ...
        setTimeout(() => {
            if (popout && popout.parentElement) {
                popout.classList.remove('show');
                setTimeout(() => popout.remove(), 400); 
            }
        }, 3500);
    ...
    renderDecisionScreen(duration, reason = null, forced = false) {
        const root = this.ensureShadow();
        if (!root) return;
        ...
        // The user expects to see why they are back at the start when they return to the tab.
        // FIX 104: Restoration of Progress Reset Toast on Decision Screen.
        // The user expects to see why they are back at the start when they return to the tab.
        const needsReset = sessionStorage.getItem(`cure_needs_reset_${window.location.hostname}`) === 'true';
        
        // FIX 112: Robust persistence. If we need a reset OR we just triggered it recently, show it.
        if (needsReset || this._resetToastPending) {
            if (needsReset) {
                sessionStorage.removeItem(`cure_needs_reset_${window.location.hostname}`);
                this._resetToastPending = true; // Mark as pending in memory
                
                // Clear the pending flag after 2s to allow for re-renders but stop it eventually
                setTimeout(() => { this._resetToastPending = false; }, 2000);
            }
            
            this.currentChallengeText = null; 

            // Only trigger the actual toast if it's not already visible/queued
            if (!this._toastDebounce) {
                ...
                }, 300); // 300ms is perfectly sufficient and snappier
            }
        }
    }
    ...
    async renderHardLock(overlay) {
        // Fix 42/45: Removed premature dismissToast() to allow reset message to persist on Decision screen
        const root = this.ensureShadow();
