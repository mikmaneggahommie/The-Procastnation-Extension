...
| Feb 4, 2026  | **Fix: Lock Logic "Short-Circuit"**         | `HEAD`                       | Fixed `evaluateAllTriggers` to respect `reminderWhitelist` and `pauseWhitelist`. Prevents UI freeze loop on allowlisted sites.                          |
...
| Feb 4, 2026  | **UI: Universal Button Parity & Alignment** | HEAD                         | Unified all 5 overlays: 56px button height, 12px radius, and 70px top padding. Stripped redundant inline overrides to ensure 100% stable visual parity. |
