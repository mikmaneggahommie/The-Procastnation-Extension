...
| :----------- | :--------------------------------------------------------- | :-------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Feb 28, 2026 | **Strict Default Settings Overhaul**                       | `HEAD`                      | Implemented stricter default behavior for the entire extension: default reminder interval reduced from 30 to 10 minutes, default tracking window for site reminders switched from 'per visit' to 'per day', and Strict Lock default limit reduced from 30 to 15 minutes. Pre-configured UI entries to match these safe-state defaults.                                              |
| Feb 28, 2026 | **Fix: Toast Hierarchy, Stack Sync & Global Heartbeat**    | `ad404f6`                   | Implemented deterministic sorting (Hierarchy: Browser > Time > Launch) for toasts. Fixed cross-tab synchronization to display full stacks on load. Added a 30s Global Background Heartbeat to prevent toasts from disappearing on whitelisted or background tabs. Increased content script pruning threshold to 10 minutes.                                                         |
| Feb 16, 2026 | **Fix: Iframe Toast Persistence & TypeError Null Guard**   | `HEAD`                      | Fixed toast recurring bug in iframes by propagating `targetHostname` from background to dismiss button. Added null guard for `ensureShadow()` in `showToast` to prevent TypeError. Updated background relay to include sender tab for dismiss broadcasts.                                                                                                                           |
| Feb 17, 2026 | **Fix: Launch Accuracy, Live Sync & Cross-Tab Dismissal**  | `HEAD`                      | Fixed launch count inaccuracy (redirect deduplication); implemented inclusive triggering (limit 2 on Visit 2); added "Live Sync" for real-time count updates; and fixed the "X" button type targeting for globally persistent dismissals across all tabs/iframes.                                                                                                                   |
| Feb 15, 2026 | **Fix: Per-Visit Reminder Never Triggering + Persistence** | `v1.0-per-visit-fix-stable` | 4-part fix in `background.js`. Root cause: `!sessionResumed` was always true when no launch-based triggers enabled → baseline reset every tick → `effectiveSecs` stayed at 0. Fixed session timeout (Pure Visit mode), `shouldReset` in trackLaunch, `isNewLaunch` in trackUsage, and added `!state.active` guard so per-visit reminders persist on new tabs instead of dismissing. |
| Feb 12, 2026 | **Fix: Configurable Session Timeout & Daily Persistence**  | `HEAD`                      | Implemented Daily-Persistent Time Snapshots and Configurable Session Reset Timeout. Users can now choose their own inactivity gap (1-1440m) for 'Per Visit' resets, while Snapshots provide a universal floor for 'Per Day' tracking. Zero-latency robustness.                                                                                                                      |
| Feb 11, 2026 | **Fix: Responsive Iframe UX (Style Sync)**                 | `47d37b0`                   | Implemented `styleOverride` pattern for reminders. Iframes now strictly respect broadcasted styles (toast/overlay) and utilize adaptive scaling (smaller fonts/padding) to fit embedded contexts.                                                                                                                                                                                   |
| Feb 11, 2026 | **Fix: Source Authority (Reminders)**                      | `5c9add6`                   | Definitive fix for "Once" reminder timing. Implemented Source Authority pattern: `trackLaunch` (0-time awareness) is forbidden from initializing Time Reminders. Only `trackUsage` (authoritative time) can establish baselines. Fixes "0-baseline" bug.                                                                                                                            |
| Feb 11, 2026 | **Fix: Settings-Change Guard**                             | `4b3eabd`                   | Implemented comprehensive "Staleness Guard" across background and content scripts. Blocks stale reminders, locks, and breathing room triggers for 2s after settings change. Eliminates race conditions and "ghost" re-locking.                                                                                                                                                      |
| Feb 9, 2026  | **Fix: Monotonic Load (Anti-Jitter)**                      | `HEAD`                      | Implemented Monotonic Logic in `loadTimer`. Prevents visual stutter/jitter by ignoring storage updates that are identical or slightly behind local timer (unless it's a reset >5s).                                                                                                                                                                                                 |
...
| Feb 9, 2026 - [x] **Milestone: Seamless Global Timer Implementation** - [x] Centralize timer authority in `background.js` (Single Source of Truth) - [x] Implement real-time broadcast of authoritative time via `timerUpdate` - [x] Fix "Time Travel" bug by disabling distributed writes in `content.js` - [x] Resolve jitter issue by increasing sync frequency to 1s - [x] Standardize hostname cleaning for consistent cross-subdomain matching

- [x] **Milestone: UI Resilience & Extension Updates**
  - [x] Replace timer with reload emoji (🔄) on extension context invalidation
- [x] **Milestone: Final Verification & "Lock In"** - [x] Verify synchronization across multiple YouTube tabs - [x] Confirm no time regressions during rapid tab switching - [x] Push authoritative fix to GitHub repositoryr & Hostname Sync** | `HEAD` | Standardized storage keys (cleanHost) to sync `www` vs non-`www` domains. Implemented monotonic time updates to prevent timer jumps backwards or cross-tab desync. |
